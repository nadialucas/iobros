...
# looks like we should drop 2 and 10
...
fit1 <- lm(lnb ~ est_min + est_max + catalog_price + grade_min + grade_max + factor(us) + factor(no_value) 
           + factor(bidder) + factor(target) + factor(auction_id), data = stamps_final)

# predicted bids
stamps_final$lnbhat <- stamps_final$lnb - (fit1$coefficients[2]*stamps_final$est_min + fit1$coefficients[3]*stamps_final$est_max +
  fit1$coefficients[4]*stamps_final$catalog_price + fit1$coefficients[5]*stamps_final$grade_min + fit1$coefficients[6]*stamps_final$grade_max + 
  fit1$coefficients[7]*stamps_final$us + fit1$coefficients[8]*stamps_final$no_value)
...
# Now integrate
A = 1/integrate(function(x) (hr_bar_fun(x)/(1-Gm_fun(x))), lower = max(min(hr_bar$x), min(gm$x)), upper = max(gm$x)-1)$value
print(A)
...
bids = seq(minbid, maxbid, by = 20)

vals = lapply(bids, v)
# lol yikes
plot(bids[vals>0], vals[vals>0])
```
